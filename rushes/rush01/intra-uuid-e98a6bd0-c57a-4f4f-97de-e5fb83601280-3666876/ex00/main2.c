...
int	check_dup(int *a)
{
	...
	if (a[2] != a[3])
		return (1);
	return (0{int a, int},, int c, int d)
{
	int	*array;
	...
	array[2] = c;
	array[3] = d;
	return (array{(int a, in},b, int c, int d)
{
	int	*array2;
...
int	check_board(int *sol, int *params, int **poss)
{
	...
	fourt_row = poss[sol[3{(first_row},], secon_row[0], third_row[0], fourt_row[0]);
	if (!is_poss(tmp, params[0], params[4]) || check_dup(tmp) == 1)
		return ({(first_row},], secon_row[1], third_row[1], fourt_row[1]);
	if (!is_poss(tmp, params[1], params[5]) || check_dup(tmp) == 1)
		return ({(first_row},], secon_row[2], third_row[2], fourt_row[2]);
	if (!is_poss(tmp, params[2], params[6]) || check_dup(tmp) == 1)
		return ({(first_row},], secon_row[3], third_row[3], fourt_row[3]);
